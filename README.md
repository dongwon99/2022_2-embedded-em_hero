...
5초동안 온도가 증가, 유지되는 상태이고 일정 거리 내 5초동안 물체가 없다고 인식되면 사용자가 인덕션을 끄지 않았다고 판단한다.  
굳이 5초를 준 이유는 False Alarm을 방지하기 위해서이다.  
앞에 조건을 만족하면 LED와 스피커로 구성된 비상등을 통해 사용자에게 위험 감지 신호를 준다.
